...
MSABrowser.prototype.highlightPosition = function(species, position) {
    ...
    $mainDiv.find('.highlight-rectangle').remove();
    $mainDiv.find('.position-number').remove();


    // Change highlight-column to be a highlight everything class
   //template1 = `<div class="highlight-column position-number" style="left:${alignmentPosition * 20}px">${position+1}</div>`;

    template2 = `<div class="highlight-rectangle" style="left:${(alignmentPosition * 20)+160}px">
                    <div class="position-number">${position+1}</div>
                </div>`;


    //$mainDiv.find('.sequence:eq(0)').append(template);
    $mainDiv.find('.scroll-container').append(template2);
    //$mainDiv.find('.sequence:eq(0)').append(template1);
...
MSABrowser.prototype.renderAnnotationData = function(annotationData){
    ...
    var width = ((parseInt(endPoint)) - (parseInt(startPoint))+3);
    var lineHTML = `
    <div id="${name}" class="annotation" data-start-point="${startPoint}" data-end-point="${endPoint}">  
    </div>
    <script>
        var data = [
            {
              x: [...Array(${width}).keys()].map(i => i + 1),
              y: [...Array(${width}).keys()].map(() => Math.random().toFixed(2)),
                mode: 'lines',
                line: {
    ...
    <script>
        var data = [
            {
              x: [...Array(2803).keys()],
              y: [...Array(2803).keys()].map(() => Math.random().toFixed(2)),
                type: 'bar',
                line: {
                  color: 'rgb(55, 128, 191)',
                  width: 1
    ...
    var HTML = '';
    if(annotationData["annotation_type"] == "line"){
        HTML = lineHTML;
    } else if(annotationData["annotation_type"] == "bar"){
        HTML = barHTML;
    } else if(annotationData["annotation_type"] == "label"){
        HTML = labelHTML;
    }
    //var annotationMessage = `<p style="width: 800px">${name} (${startPoint} - ${endPoint})</p>`.repeat(repeatCount);
    

    // Create and render the box chart on the canvas
    ...
    <script>
        var canva = document.getElementById("${name}").getContext("2d");
        </script>
        parseInt($(this).data('start-point')) - 1


                $(this).css('display', 'flex');
        $(this).css('left', ((parseInt(${startPoint}) - 1) * 20) + 'px');
